...
{
    public static class ProjectListQuery
    {
                ...
                if (request.Roles.Contains(Roles.MeetingPlanner))
                {
                    result.AllProjects = await _db.Projects
                        .Where(p => p.ProjectType == Data.Enums.ProjectTypeEnum.Project &&
                            p.Id != 1 && p.IsActive == true && (p.UserProjectMapping.Any(upm => upm.UserId.Equals(request.UserId)) || p.CreatedByUserId == request.UserId))
